...
try:
  ...
  if input_size and 1.01 > float(output_size) / float(input_size) > 0.99:
    ...
    #
    cmd = [FFMPEG_PATH, '-i', os.path.join(temp_dir, video_basename), '-crf', '18', '-map', '0', '-acodec', 'copy', '-scodec', 'cpoy', '-c:v', 'libx264', '-threads', '0', '-preset', 'slow', os.path.join(temp_dir_b, video_basename)]
    subprocess.call(cmd)
    #
    #
    #
